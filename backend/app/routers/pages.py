...
@router.post("/", response_model=schemas.PageResponse, status_code=status.HTTP_201_CREATED)
def create_page(
    page: schemas.PageCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    # Verify the parent binder actually exists before creating a page for it
...
def read_pages_for_binder(
    ...
    db_binder = crud_binder.get_binder(db, binder_id)
    if not db_binder:
...
def update_page(
    ...
    return crud_page.update_page(db, page_id, page_update)
...
def delete_page(
    ...
    if db_page.binder.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to modify this page")
    crud_page.delete_page(db, page_id)
